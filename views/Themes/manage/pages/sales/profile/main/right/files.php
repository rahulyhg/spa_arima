...
$options = array(
	'url' => URL."media/lists?obj_type=emp_file&obj_id={$this->item['id']}"
);

if( !empty($this->item['id']) == $this->me['id'] || !empty($this->permit['sales']['edit']) ){
	$options['upload_url'] = URL."media/set?obj_type=emp_file&obj_id={$this->item['id']}";
	$options['remove_url'] = URL."media/del/";
}

?><div id="posts" data-plugins="rupload" data-options="<?=$this->fn->stringify($options);?>">
<?php if( !empty($this->item['id']) == $this->me['id'] || !empty($this->permit['sales']['edit']) ){ ?>
