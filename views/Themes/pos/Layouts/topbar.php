...
$this->nav = array();
$this->nav[] = array('id'=>'orders','name'=> $this->lang->translate('menu','Orders').'<span class="mls countVal">(5)</span>', 'icon'=>'file-text-o','url'=>URL.'pos');
$this->nav[] = array('id'=>'booking','name'=> $this->lang->translate('menu','Booking').'<span class="mls countVal">(5)</span>', 'icon'=>'address-book-o','url'=>URL.'pos');
$this->nav[] = array('id'=>'members','name'=> $this->lang->translate('menu','Members').'<span class="mls countVal">(5)</span>', 'icon'=>'address-card-o','url'=>URL.'pos');
$this->nav[] = array('id'=>'queue','name'=> $this->lang->translate('menu','Queue').'<span class="mls countVal">(5)</span>', 'icon'=>'list-ol','url'=>URL.'pos');
...
foreach ($this->nav as $key => $value) {
	...
	$active = $this->getPage('on')==$value['id'] ? ' class="active"':'';
	$pageNav .= '<li id="global-nav-'.$value['id'].'" '.$active.' data-global-action="'.$value['id'].'"><a href="'.$value['url'].'" data-nav="'.$value['id'].'"><i class="icon-'.$value['icon'].'"></i><strong>'.$value['name'].'</strong></a></li>';
}
