	...
	var Order = {
		    ...
		    self.$elem.find('[data-bill-set]').click( function () {
		    	...
		    	Dialog.load( Event.URL + 'orders/set_bill', data, {
		    		...
		    		onSubmit: function ( $el ) {
		    			...
		    			if(  type=='drink' ){
		    				self.currOrder.summary.drink = parseInt( $.trim($el.$pop.find('#drink').val()) ) || 0;
			    			self.summaryPrice();
			    			self.summaryDisplay();
		...
		invoice: {
			...
			buildFrag: function ( orderData ) {
				...
				self.data = {
					...
					summary: {
						...
						pay: parseInt( orderData.pay ),
						change: parseInt( orderData.change ),
					},

					number: orderData.number,
					number_str: orderData.number_str,
			...
			display: function () {
				...
				self.$elem.find( '[data-invoice=number]' ).text( self.data.number_str );
				self.$elem.find( '[data-invoice=status]' ).html( self.then.setStatus( self.data.status ) );
			...
			Events: function () {
				...
				self.$elem.find('[data-invoice-action]').click( function () {
					...
					else if( type=='drink' || type=='vip' ) {
						self.updateSummary( type );
					}
			...
			calculateMoney: function () {
				...
				self.data.summary.total = self.data.summary.subtotal + self.data.summary.drink + self.data.summary.room_price;

				self.data.summary.balance = self.data.summary.total - (self.data.summary.discount + self.data.summary.pay);

				self.data.summary.change = self.data.summary.pay>self.data.summary.balance
					? self.data.summary.pay - self.data.summary.balance
					: 0;

				console.log( self.data.summary );
			},

			updateSummary: function ( type ) {
				var self = this;

				var dataPost = {type: type}
				if( type=='vip' ){
					dataPost.room_price = self.data.summary.room_price;
				}
				if( type=='drink' ){
					dataPost.drink = self.data.summary.drink;
				}

				Dialog.load( Event.URL + 'orders/set_bill', dataPost, {

					onSubmit: function ( $d ) {
						var $form = $d.$pop.find('form');

						if( type=='vip' ){
							self.data.summary.room_price = parseInt( $form.find( '[name=room_price]' ).val() ) || 0;
						}

						if( type=='drink' ){
							self.data.summary.drink = parseInt( $form.find( '[name=drink]' ).val() ) || 0;
						}

						self.calculateMoney();

						self.saveOrder( self.data.id, self.data.summary ).always(function () {
							Dialog.close();
			...
			saveOrder: function ( id, dataPost ) {
				return $.ajax({
					method: "POST",
					url: Event.URL + 'orders/update/'+ id,
					data: dataPost,
					dataType: 'json'
