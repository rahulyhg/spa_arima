<div class="slipPaper-container">
	<div class="slipPaper-main">
		<div class="slipPaper-content clearfix">
			<div class="slipPaper-bodyHeader">

				<div class="clearfix">
					<div class="orderID"><span header="code"></span></div>
					<div class="lfloat tableName">
						<div header="title">โต๊ะ 1</div>
						<div header="countguest" class="tableTime-text fsm fwn">จำนวนลูกค้า 5 คน</div>
						<div header="itmelive" class="tableTime-text fsm fwn">18, 5:32:24</div>
					</div>
					<div class="rfloat">
						<span class="gbtn radius"><a class="js-add-item btn btn-blue"><i class="icon-plus"></i></a></span>
					</div>
				</div>

				
			</div>
			<div class="slipPaper-bodyContent" style="padding-top: 29px; top: 85px; bottom: 164px;">
				<div class="slipPaper-bodyContent-header" style="padding-right: 5px;">
					<table>
						<tbody><tr>
							<th class="no"></th>
							<th class="name">รายการ</th>
							<th class="qty">จำนวน</th>
							<th class="price">ราคา</th>
							<th class="total">รวม</th>
						</tr>
					</tbody></table>
				</div>
				<div class="slipPaper-bodyContent-body" style="height: 228px; overflow-y: auto;"><table role="orderlists"></table>
				</div>
			</div>
			<div class="slipPaper-bodyFooter">
				<table class="slipPaper-bodyFooter-summary mvm">
					<tbody><tr>
						<td class="colleft">
							<table>
								<tbody><tr>
									<td class="label">ส่วนลด:</td>
									<td class="data">฿<span summary="total-discount-text">0</span></td>
								</tr>
								<tr>
									<td class="label">รวม:</td>
									<td class="data">฿<span summary="subtotal-text">227</span></td>
								</tr>
								<tr>
									<td class="label">ค่าบริการ:</td>
									<td class="data">฿<span summary="service-text">0</span></td>
								</tr>
							</tbody></table>
						</td>
						<td class="colright">
							<table>
								<tbody><tr>
									<td class="label">ภาษี:</td>
									<td class="data">฿<span summary="total-tax-text">0</span></td>
								</tr>
								<tr>
									<td class="label">รวมทั้งหมด:</td>
									<td class="data TOTAL">฿<span summary="total-text">227</span></td>
								</tr>
								<tr>
									<td class="label">จำนวนที่ต้องจ่าย:</td>
									<td class="data Balance">฿<span summary="balance-text">0</span></td>
								</tr>
							</tbody></table>
						</td>
					</tr>
				</tbody></table>

				<div class="slipPaper-bodyFooter-actions mvm clearfix">
					<div class="lfloat">
						<span class="gbtn"><a href="http://localhost/foods/cashier" class="btn"><i class="icon-arrow-left mrs"></i>กลับ</a></span>
						<!-- <span class="gbtn"><a class="btn"><i class="icon-envelope"></i></a></span> -->
					</div>
					<div class="rfloat">
						
						
						<span class="gbtn"><a class="js-cancel-order btn disabled">ยกเลิก</a></span>
						<span class="gbtn"><a class="js-send-order btn btn-blue disabled">สั่งอาหาร</a></span>
					</div>
				</div>
			</div>
		</div>
	</div>
	<footer class="slipPaper-footer">
		
		<nav>
			<ul class="clearfix" style="width:800px">
										
				<li class="button">
					<span class="gbtn radius"><a class="btn js-set-option" data-type=""><i class="icon-file-text-o"></i></a></span>
					<span class="t">เพิ่มข้อมูลสมาชิก</span>
				</li>
										
				<li class="button">
					<span class="gbtn radius"><a class="btn js-set-option" data-type=""><i class="icon-retweet"></i></a></span>
					<span class="t">ย้ายโต๊ะ</span>
				</li>
										
				<li class="button">
					<span class="gbtn radius"><a class="btn js-set-option" data-type="changeGuest"><i class="icon-users"></i></a></span>
					<span class="t">เปลี่ยนจำนวนลูกค้า</span>
				</li>
								</ul>
		</nav>

		<div class="pay button">
			<span class="gbtn radius large"><a class="btn btn-blue js-payment disabled"><i class="icon-thailand-baht"></i></a></span>
			<span class="t">Pay</span>
		</div>
	</footer>
</div>