...
class Stocks extends Controller {
    ...
    public function index($id=null) {
    	$this->view->setPage('on', 'stocks' );
        if( !empty($id) ){
            
            $item = $this->model->query('models')->get($id, array('items' => true));
            if(empty($item))$this->error ();

            $color = $this->model->query('models')->getColors($id);

            if( $this->format=='json' ){

                $this->view->setData('results', $this->model->query('products')->lists( array('model'=>$id) ) ); 
                $render = "stocks/profile/lists/json";
            }
            else{
                $render = "stocks/profile/display";
            }

            $this->view->setData('status', array() ); 
