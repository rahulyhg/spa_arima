...
(function( $, window, document, undefined ) {
	...
	var slidesImages = {
		init: function ( options, elem ) {
			...
			self.options = $.extend( {}, $.fn.slidesImages.options, options );
			self.setup();
		...
		setup: function(){
			...
			self.limit = self.$elem.find('.slide-images>li').length;

			self.inx = -1;
			self.$ol = self.$elem.find('.slide-images');
		...
		display: function( length ){
			...
			self.effect[ self.options.effect ](self, length);
			self.is_first = false;
	...
	$.fn.slidesImages = function( options ) {
		return this.each(function() {
			var $this = Object.create( slidesImages );
			$this.init( options, this );
			$.data( this, 'slidesImages', $this );
		});
	};

	$.fn.slidesImages.options = {
		speed: 500,
